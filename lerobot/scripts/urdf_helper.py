...
def link_to_world_transform(
    ...
    for i in range(1, link + 1):
        ...
        angle_rad = 0 if i - 1 >= len(joint_angles) else joint_angles[i - 1]
...
#     for vel, name in zip(cartesian_velocity, POS_DIM_NAMES, strict=False):
...
def path_to_link(link: int) -> str:
    return "/".join(f"link_{i}" for i in range(link + 1))
...
def log_angle_rot(
    ...
    rr.log(entity_path, rr.Transform3D(translation=start_translation, mat3x3=rotation_mat))
